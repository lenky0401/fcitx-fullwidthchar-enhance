                             ...
                             )
{
    ...
    //fprintf(stderr, "profile->bUseFullWidthChar:%d\n", profile->bUseFullWidthChar);
...
char* ProcessFullWidthChar(void* arg, const char* str)
{
    ...
    if (profile->bUseFullWidthChar/* && status->visible*/) {
        ...
        for (; i < len; ++i) {
            ...
            sym = FcitxHotkeyPadToMain((FcitxKeySym)ps);
            if (chr_len == 1 && sym >= '\x20' && sym <= '\x7e') {
                strcat(ret, sCornerTrans[sym - 32]);
                ret_len += strlen(sCornerTrans[sym - 32]);
            } else {
